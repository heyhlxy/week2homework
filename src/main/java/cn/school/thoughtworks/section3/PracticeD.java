...
public class PracticeD {
    Map<String, Integer> createUpdatedCollection (List<String> collectionA, Map<String, List<String>> object) {
        // 统计出collection1中相同的元素的个数，如果有-就把-右边的数字也计算入个数，形成collection3，
        ...
        for (String string : collectionA) {
            if (string.contains("-")) {
                ...
                if (stringValue != null) {
                    if (collection3.get(stringKey) != null) {
                        collection3.put(stringKey,inteValue + collection3.get(stringKey).intValue()); // 考虑字符串顺序
                    } else {
                        collection3.put(stringKey, inteValue);
                    }
                }
        ...
        // 选出collection3中的元素的key属性跟collection2中value属性对应的list中的元素相同的元素
